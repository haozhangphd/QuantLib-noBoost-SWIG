...
typedef std::shared_ptr<Instrument> BondPtr;
typedef std::shared_ptr<Instrument> ZeroCouponBondPtr;
typedef std::shared_ptr<Instrument> FixedRateBondPtr;
typedef std::shared_ptr<Instrument> FloatingRateBondPtr;
typedef std::shared_ptr<PricingEngine> DiscountingBondEnginePtr;
%}

%rename(Bond) BondPtr;
class BondPtr : public std::shared_ptr<Instrument> {
  ...
  public:
    %extend {
        ...
        Rate nextCouponRate(const Date& d = Date()) {
            return std::dynamic_pointer_cast<Bond>(*self)
                ->nextCouponRate();
        }
        ...
        Date settlementDate(Date d = Date()) {
            return std::dynamic_pointer_cast<Bond>(*self)->settlementDate(d);
        }
        Date startDate() const {
            return std::dynamic_pointer_cast<Bond>(*self)->startDate();
        }
        Date maturityDate() const {
            return std::dynamic_pointer_cast<Bond>(*self)->maturityDate();
        }
        Date issueDate() const {
            return std::dynamic_pointer_cast<Bond>(*self)->issueDate();
        }
        std::vector<std::shared_ptr<CashFlow> > cashflows() const {
            return std::dynamic_pointer_cast<Bond>(*self)->cashflows();
        }
        std::vector<std::shared_ptr<CashFlow> > redemptions() const {
            return std::dynamic_pointer_cast<Bond>(*self)->redemptions();
        }
        std::shared_ptr<CashFlow> redemption() const {
            return std::dynamic_pointer_cast<Bond>(*self)->redemption();
        }
        Calendar calendar() const {
            return std::dynamic_pointer_cast<Bond>(*self)->calendar();
        }
        std::vector<Real> notionals() const {
            return std::dynamic_pointer_cast<Bond>(*self)->notionals();
        }
        Real notional(Date d = Date()) const {
            return std::dynamic_pointer_cast<Bond>(*self)->notional(d);
        }
        // calculations
        Real cleanPrice() {
            return std::dynamic_pointer_cast<Bond>(*self)->cleanPrice();
        }
        ...
        Real dirtyPrice() {
            return std::dynamic_pointer_cast<Bond>(*self)->dirtyPrice();
        }
  ...
  public:
    %extend {
        ...
        Frequency frequency() const {
            return std::dynamic_pointer_cast<FixedRateBond>(*self)
                ->frequency();
        }
...
typedef std::shared_ptr<Instrument> CmsRateBondPtr;
%}
...
class DiscountingBondEnginePtr : public std::shared_ptr<PricingEngine> {
  public:
    %extend {
...
typedef std::shared_ptr<Instrument> CallableFixedRateBondPtr;
typedef std::shared_ptr<PricingEngine> TreeCallableFixedRateBondEnginePtr;
typedef std::shared_ptr<PricingEngine> BlackCallableFixedRateBondEnginePtr;
%}
  ...
  public:
    %extend {
        CallableFixedRateBondPtr(
                ...
                Real redemption,
                Date issueDate,
                const std::vector<std::shared_ptr<Callability> > &putCallSchedule) {
...
typedef std::shared_ptr<Instrument> CPIBondPtr;
%}
    ...
    %extend {
        CPIBondPtr(
            ...
            std::shared_ptr<ZeroInflationIndex> zeroIndex =
                std::dynamic_pointer_cast<ZeroInflationIndex>(cpiIndex);
