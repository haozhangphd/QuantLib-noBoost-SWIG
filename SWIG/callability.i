...
typedef std::shared_ptr<Callability> CallabilityPtr;
typedef std::shared_ptr<Callability> SoftCallabilityPtr;
...
%template(Callability) std::shared_ptr<Callability>;
%extend std::shared_ptr<Callability> {
    shared_ptr<Callability>(const CallabilityPrice& price,
                            Callability::Type type,
                            const Date& date) {
        return new std::shared_ptr<Callability>(
...
class SoftCallabilityPtr : public std::shared_ptr<Callability> {
  public:
    %extend {
...
SWIG_STD_VECTOR_ENHANCED( std::shared_ptr<Callability> )
#endif
namespace std {
    %template(CallabilitySchedule) vector<std::shared_ptr<Callability> >;
}
