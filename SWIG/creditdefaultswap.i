...
typedef std::shared_ptr<Instrument> CreditDefaultSwapPtr;
typedef std::shared_ptr<PricingEngine> MidPointCdsEnginePtr;
typedef std::shared_ptr<PricingEngine> IntegralCdsEnginePtr;
%}

%rename(CreditDefaultSwap) CreditDefaultSwapPtr;
class CreditDefaultSwapPtr : public std::shared_ptr<Instrument> {
  public:
    %extend {
        ...
        Protection::Side side() const {
            return std::dynamic_pointer_cast<CreditDefaultSwap>(*self)
                ->side();
        }
        Real notional() const {
            return std::dynamic_pointer_cast<CreditDefaultSwap>(*self)
        ...
        doubleOrNull upfront() const {
            std::optional<Rate> result =
                std::dynamic_pointer_cast<CreditDefaultSwap>(*self)
                ->upfront();
            if (result)
                return *result;
            else
        ...
        std::vector<std::shared_ptr<CashFlow> > coupons() {
            return std::dynamic_pointer_cast<CreditDefaultSwap>(*self)
                ->coupons();
        }
    }
...
class MidPointCdsEnginePtr : public std::shared_ptr<PricingEngine> {
  public:
    %extend {
...
class IntegralCdsEnginePtr : public std::shared_ptr<PricingEngine> {
  public:
    %extend {
