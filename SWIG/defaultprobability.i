...
IsObservable(std::shared_ptr<DefaultProbabilityTermStructure>);
...
typedef std::shared_ptr<DefaultProbabilityTermStructure> FlatHazardRatePtr;
%}

%rename(FlatHazardRate) FlatHazardRatePtr;
class FlatHazardRatePtr
    : public std::shared_ptr<DefaultProbabilityTermStructure> {
  public:
    %extend {
...
%{
typedef std::shared_ptr<DefaultProbabilityTermStructure> Name##Ptr;
%}

%rename(Name) Name##Ptr;
class Name##Ptr : public std::shared_ptr<DefaultProbabilityTermStructure> {
  public:
    %extend {
        ...
        const std::vector<Real>& hazardRates() {
            typedef InterpolatedHazardRateCurve<Interpolator> Name;
            return std::dynamic_pointer_cast<Name>(*self)->hazardRates();
        }
  ...
  public:
    %extend {
        ...
        const std::vector<Real>& defaultDensities() {
            typedef InterpolatedDefaultDensityCurve<Interpolator> Name;
            return std::dynamic_pointer_cast<Name>(*self)->defaultDensities();
        }
        #if !defined(SWIGR) && !defined(SWIGGUILE) && !defined(SWIGMZSCHEME)
        std::vector<std::pair<Date,Real> > nodes() {
            typedef InterpolatedDefaultDensityCurve<Interpolator> Name;
...
typedef std::shared_ptr<DefaultProbabilityHelper> SpreadCdsHelperPtr;
using QuantLib::UpfrontCdsHelper;
typedef std::shared_ptr<DefaultProbabilityHelper> UpfrontCdsHelperPtr;
%}

// rate helpers for curve bootstrapping
%template(DefaultProbabilityHelper) std::shared_ptr<DefaultProbabilityHelper>;

#if defined(SWIGCSHARP)
SWIG_STD_VECTOR_ENHANCED( std::shared_ptr<DefaultProbabilityHelper> )
#endif
namespace std {
    %template(DefaultProbabilityHelperVector)
    vector<std::shared_ptr<DefaultProbabilityHelper> >;
}


%rename(SpreadCdsHelper) SpreadCdsHelperPtr;
class SpreadCdsHelperPtr : public std::shared_ptr<DefaultProbabilityHelper> {
  public:
    %extend {
        SpreadCdsHelperPtr(
...
class UpfrontCdsHelperPtr : public std::shared_ptr<DefaultProbabilityHelper> {
  public:
    %extend {
        UpfrontCdsHelperPtr(
  ...
  public:
    %extend {
        Name##Ptr(
              ...
              const std::vector<std::shared_ptr<DefaultProbabilityHelper> >&
                                                                  instruments,
              const DayCounter& dayCounter,
