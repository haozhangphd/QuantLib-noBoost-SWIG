...
%template(Dividend) std::shared_ptr<Dividend>;

%{
using QuantLib::FixedDividend;
using QuantLib::FractionalDividend;

typedef std::shared_ptr<Dividend> FixedDividendPtr;
typedef std::shared_ptr<Dividend> FractionalDividendPtr;
%}

%rename(FixedDividend) FixedDividendPtr;
class FixedDividendPtr : public std::shared_ptr<Dividend> {
  public:
    %extend {
...
class FractionalDividendPtr : public std::shared_ptr<Dividend> {
  public:
    %extend {
...
SWIG_STD_VECTOR_ENHANCED( std::shared_ptr<Dividend> )
#endif
namespace std {
    %template(DividendSchedule) vector<std::shared_ptr<Dividend> >;
}
