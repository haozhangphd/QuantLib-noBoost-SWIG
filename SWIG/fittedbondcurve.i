...
typedef std::shared_ptr<YieldTermStructure> FittedBondDiscountCurvePtr;
typedef QuantLib::FittedBondDiscountCurve::FittingMethod FittingMethod;

std::vector<std::shared_ptr<BondHelper> > convert_bond_helpers(
                 const std::vector<std::shared_ptr<RateHelper> >& helpers) {
    std::vector<std::shared_ptr<BondHelper> > result(helpers.size());
    for (Size i=0; i<helpers.size(); ++i)
        result[i] = std::dynamic_pointer_cast<BondHelper>(helpers[i]);
    return result;
}
%}
  ...
  public:
    %extend {
        ...
        const FittingMethod& fitResults() const {
            return std::dynamic_pointer_cast<FittedBondDiscountCurve>(*self)
                ->fitResults();
        }
    }
};
