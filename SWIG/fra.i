...
typedef std::shared_ptr<Instrument> ForwardRateAgreementPtr;
%}
...
class ForwardRateAgreementPtr : public std::shared_ptr<Instrument> {
  public:
    %extend {
        ...
        Real spotIncome(const Handle<YieldTermStructure>& discount) const {
            return std::dynamic_pointer_cast<ForwardRateAgreement>(*self)
                ->spotIncome(discount);
        }
