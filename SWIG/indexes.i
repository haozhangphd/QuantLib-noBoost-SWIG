...
%template(Index) std::shared_ptr<Index>;
%extend std::shared_ptr<Index> {
    #if defined(SWIGRUBY)
...
IsObservable(std::shared_ptr<Index>);

// interest-rate indexes
%{
using QuantLib::InterestRateIndex;
typedef std::shared_ptr<Index> InterestRateIndexPtr;
%}

%rename(InterestRateIndex) InterestRateIndexPtr;
class InterestRateIndexPtr : public std::shared_ptr<Index> {
  ...
  public:
    %extend {
        std::string familyName() {
            return std::dynamic_pointer_cast<InterestRateIndex>(*self)
                ->familyName();
        }
        Period tenor() {
            return std::dynamic_pointer_cast<InterestRateIndex>(*self)
...
typedef std::shared_ptr<Index> IborIndexPtr;
typedef std::shared_ptr<Index> OvernightIndexPtr;
%}
  ...
  public:
    %extend {
        ...
        BusinessDayConvention businessDayConvention() {
            return std::dynamic_pointer_cast<IborIndex>(*self)
                ->businessDayConvention();
        }
        bool endOfMonth() {
            return std::dynamic_pointer_cast<IborIndex>(*self)->endOfMonth();
        }
...
%inline %{
    IborIndexPtr as_iborindex(const InterestRateIndexPtr& index) {
        return std::dynamic_pointer_cast<IborIndex>(index);
    }
%}
...
typedef std::shared_ptr<Index> LiborPtr;
%}
...
typedef std::shared_ptr<Index> SwapIndexPtr;
%}
  ...
  public:
    %extend {
        SwapIndexPtr(const std::string& familyName,
            ...
            std::shared_ptr<IborIndex> xibor =
                std::dynamic_pointer_cast<IborIndex>(iborIndex);
        ...
        Period fixedLegTenor() {
            return std::dynamic_pointer_cast<SwapIndex>(*self)
                ->fixedLegTenor();
        }
        BusinessDayConvention fixedLegConvention() {
            return std::dynamic_pointer_cast<SwapIndex>(*self)
