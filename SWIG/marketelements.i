...
%template(Quote) std::shared_ptr<Quote>;
IsObservable(std::shared_ptr<Quote>);
...
typedef std::shared_ptr<Quote> SimpleQuotePtr;
%}

%rename(SimpleQuote) SimpleQuotePtr;
class SimpleQuotePtr : public std::shared_ptr<Quote> {
    #if defined(SWIGRUBY)
  ...
  public:
    %extend {
        ...
        void setValue(Real value) {
            std::dynamic_pointer_cast<SimpleQuote>(*self)->setValue(value);
        }
    }
};
...
typedef std::shared_ptr<Quote> DerivedQuotePtr;
typedef std::shared_ptr<Quote> CompositeQuotePtr;
%}

%rename(DerivedQuote) DerivedQuotePtr;
class DerivedQuotePtr : public std::shared_ptr<Quote> {
  public:
    %extend {
...
class CompositeQuotePtr : public std::shared_ptr<Quote> {
  public:
    %extend {
        #if defined(SWIGPYTHON)
...
SWIG_STD_VECTOR_ENHANCED( std::shared_ptr<Quote> )
SWIG_STD_VECTOR_ENHANCED( Handle<Quote> )
SWIG_STD_VECTOR_ENHANCED( RelinkableHandle<Quote> )
#endif
namespace std {
    %template(QuoteVector) vector<std::shared_ptr<Quote> >;
    %template(QuoteVectorVector) vector<vector<std::shared_ptr<Quote> > >;
