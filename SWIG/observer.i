...
%template(Observable) std::shared_ptr<Observable>;
...
%extend Type {
    ...
    std::shared_ptr<Observable>* asObservable() {
        return new std::shared_ptr<Observable>(*self);
    }
    #else
    std::shared_ptr<Observable> asObservable() {
        return std::shared_ptr<Observable>(*self);
    }
    #endif
}
%enddef
