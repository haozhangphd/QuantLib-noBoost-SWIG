...
%template(StochasticProcess) std::shared_ptr<StochasticProcess>;
IsObservable(std::shared_ptr<StochasticProcess>);

#if defined(SWIGCSHARP)
SWIG_STD_VECTOR_ENHANCED( std::shared_ptr<StochasticProcess> )
#endif
...
typedef std::shared_ptr<StochasticProcess> StochasticProcess1DPtr;
%}

%rename(StochasticProcess1D) StochasticProcess1DPtr;
class StochasticProcess1DPtr
    : public std::shared_ptr<StochasticProcess> {
  public:
    %extend {
      Real x0() {
          return std::dynamic_pointer_cast<StochasticProcess1D>(*self)->x0();
      }
      Real drift(Time t, Real x) {
          return std::dynamic_pointer_cast<StochasticProcess1D>(*self)
              ->drift(t, x);
      }
...
typedef std::shared_ptr<StochasticProcess> GeneralizedBlackScholesProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> BlackScholesProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> BlackScholesMertonProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> BlackProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> GarmanKohlagenProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> Merton76ProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> StochasticProcessArrayPtr;
%}

%rename(StochasticProcessArray) StochasticProcessArrayPtr;
class StochasticProcessArrayPtr : public std::shared_ptr<StochasticProcess> {
  public:
    %extend {
      StochasticProcessArrayPtr(
               const std::vector<std::shared_ptr<StochasticProcess> >&array,
               const Matrix &correlation) {
          std::vector<std::shared_ptr<StochasticProcess1D> > in_array;
          for (Size j=0; j < array.size(); j++)
              in_array.push_back(
                  std::dynamic_pointer_cast<StochasticProcess1D>(array[j]));
...
typedef std::shared_ptr<StochasticProcess> GeometricBrownianMotionProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> VarianceGammaProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> HestonProcessPtr;
%}

%rename(HestonProcess) HestonProcessPtr;
class HestonProcessPtr : public std::shared_ptr<StochasticProcess> {
  public:
    %extend {
...
typedef std::shared_ptr<StochasticProcess> BatesProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> HullWhiteProcessPtr;
%}
...
typedef std::shared_ptr<StochasticProcess> GsrProcessPtr;
%}
...
class GsrProcessPtr : public StochasticProcess1DPtr {
    ...
    %extend {
        ...
        Real sigma(Time t) {
            return std::dynamic_pointer_cast<GsrProcess>(*self)->sigma(t);
        }
        Real reversion(Time t){
            return std::dynamic_pointer_cast<GsrProcess>(*self)->reversion(t);
        }
        Real y(Time t) {
            return std::dynamic_pointer_cast<GsrProcess>(*self)->y(t);
        }
        Real G(Time t, Time T, Real x){
            return std::dynamic_pointer_cast<GsrProcess>(*self)->G(t, T, x);
        }
        void setForwardMeasureTime(Time t) {
            std::dynamic_pointer_cast<GsrProcess>(*self)->setForwardMeasureTime(t);
        }
    }
};
...
%inline %{
    GsrProcessPtr as_gsr_process(
                           const std::shared_ptr<StochasticProcess>& proc) {
        return std::dynamic_pointer_cast<GsrProcess>(proc);
    }
%}
