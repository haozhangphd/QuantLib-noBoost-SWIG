...
typedef std::shared_ptr<Instrument> SwapPtr;
typedef std::shared_ptr<Instrument> VanillaSwapPtr;
typedef std::shared_ptr<Instrument> NonstandardSwapPtr;
typedef std::shared_ptr<PricingEngine> DiscountingSwapEnginePtr;
typedef std::shared_ptr<Instrument> FloatFloatSwapPtr;
%}

%rename(Swap) SwapPtr;
class SwapPtr : public std::shared_ptr<Instrument> {
  public:
    %extend {
        SwapPtr(const std::vector<std::shared_ptr<CashFlow> >& firstLeg,
                const std::vector<std::shared_ptr<CashFlow> >& secondLeg) {
            return new SwapPtr(new Swap(firstLeg, secondLeg));
        }
        Date startDate() {
            return std::dynamic_pointer_cast<Swap>(*self)->startDate();
        }
        Date maturityDate() {
            return std::dynamic_pointer_cast<Swap>(*self)->maturityDate();
        }
        const Leg & leg(Size i){
            return std::dynamic_pointer_cast<Swap>(*self)->leg(i);
        }
        Real legNPV(Size j) const {
            return std::dynamic_pointer_cast<Swap>(*self)->legNPV(j);
        }
    }
};
...
class VanillaSwapPtr : public SwapPtr {
  ...
  public:
    %extend {
        ...
        Rate fairRate() {
            return std::dynamic_pointer_cast<VanillaSwap>(*self)->fairRate();
        }
        Spread fairSpread() {
            return std::dynamic_pointer_cast<VanillaSwap>(*self)
                 ->fairSpread();
        }
        Real fixedLegBPS() {
            return std::dynamic_pointer_cast<VanillaSwap>(*self)
...
class NonstandardSwapPtr : public SwapPtr {
  ...
  public:
    %extend {
        ...
        // Inspectors
        const Leg& fixedLeg() {
            return std::dynamic_pointer_cast<NonstandardSwap> (*self)
                ->fixedLeg();
        }
        const Leg& floatingLeg() {
            return std::dynamic_pointer_cast<NonstandardSwap> (*self)
...
class DiscountingSwapEnginePtr : public std::shared_ptr<PricingEngine> {
  public:
    %extend {
...
typedef std::shared_ptr<Instrument> AssetSwapPtr;
%}
...
class AssetSwapPtr : public SwapPtr {
  ...
  public:
    %extend {
        ...
        Real fairCleanPrice() {
            return std::dynamic_pointer_cast<AssetSwap>(*self)
                ->fairCleanPrice();
        }
        Spread fairSpread() {
            return std::dynamic_pointer_cast<AssetSwap>(*self)
    ...
    %extend {
        FloatFloatSwapPtr(VanillaSwap::Type type, const std::vector<Real> &nominal1,
            ...
            std::shared_ptr<InterestRateIndex> index1 =
                std::dynamic_pointer_cast<InterestRateIndex>(indexPtr1);
            std::shared_ptr<InterestRateIndex> index2 =
                std::dynamic_pointer_cast<InterestRateIndex>(indexPtr2);
